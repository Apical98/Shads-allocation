...
SBIjuveAa=data.frame(MjuveAa2,row.names=c("Minho","Ulla","Eo","Lérez","Tambre","Nalón","Sella","Deva","Pas",
                                          "Asón","Lima","Mondego1","Mondego2","Tagus","Vouga1","Vouga2",
...
SBIjuveAf=data.frame(MjuveAf2,row.names=c("Minho","Ulla","Eo","Lérez","Tambre","Nalón","Sella","Deva","Pas",
                                          "Asón","Lima","Mondego1","Mondego2","Tagus","Vouga1","Vouga2",
                                          "Bidasoa","Oiartzun","Urumea","Oria"))
...
levels(JAa$River)[c(5,9,13)]=c("Asón","Nalón","Lérez")
...
levels(JAf$River)[c(5,9,13)]=c("Asón","Nalón","Lérez")
...
PieChart(x, hole = 0.5, stat = "%", data = df.AaFoz, fill = Fozcol, 
         main = "",color = Fozcol,line_width = 0.1,labels_size = 1.5,
...
PieChart(x, hole = 0.5, stat = "%", data = df.AaGuarda, fill = Guacol, 
         main = "",color = Guacol,line_width = 0.1,labels_size = 1.5,
...
Fiscol=c("#EEB422","#EE5C42","#8B4726","#9AFF9A")

windows()
PieChart(x, hole = 0.5, stat = "%", data = df.AaFisterra, fill = Fiscol, 
         main = "",color = Fiscol,line_width = 0.1,labels_size = 1.5,
         labels_color = c(rep("black", 2), "white","black","black"),)

## Malpica
df.AaMalpica=data.frame(x =c(rep("Tagus",2),
                             rep("Lérez",1),
                             rep("Ulla",4),
                             rep("Miño",2),
                             rep("Tambre",1),
...
PieChart(x, hole = 0.5, stat = "%", data = df.AaMalpica, fill = Malcol, 
         main = "",color = Malcol,line_width= 0.1,labels_size = 1.5,
...
PieChart(x, hole = 0.5, stat = "%", data = df.AaCoru, fill = Corcol, 
         main = "",color = Corcol,line_width = 0.1,labels_size = 1.5,
...
## A Guarda
df.AfGuarda=data.frame(x =c(rep("Ulla",1),
                            rep("Miño",3),
                            rep("Nalón",1)))

AfGuacol=c("#EEB422","#4EEE94","#B3EE3A")

windows()
PieChart(x, hole = 0.5, stat = "%", data = df.AfGuarda, fill = AfGuacol, 
         main = "",color = AfGuacol,line_width = 0.1,labels_size = 1.5,
         labels_color = c("black","black","black"),)


## Malpica
df.AfMalpica=data.frame(x =c(rep("Ulla",7),
                             rep("Tagus",4)))

AfMalcol=c("#B3EE3A","#8B3626")

windows()
PieChart(x, hole = 0.5, stat = "%", data = df.AfMalpica, fill = AfMalcol, 
         main = "",color = AfMalcol,line_width = 0.1,labels_size = 1.5,
         labels_color = c("black","white"))

## Coru
df.AfCoru=data.frame(x =c(rep("Tagus",4),
                          rep("Ulla",6),
                          rep("Miño",2)))

AfCorcol=c("#EEB422","#8B3626","#B3EE3A")

windows()
PieChart(x, hole = 0, stat = "%", data = df.AfCoru, fill = AfCorcol, 
         main = "",color = AfCorcol,line_width = 0.1,labels_size = 1.5,
...
rios <- c("Bidasoa", "Oiartzun", "Urumea", "Oria", "Asón", "Pas", "Deva", "Sella", 
          "Nalón", "Eo", "Tambre", "Ulla", "Lérez", "Minho", "Lima", "Vouga1", 
          "Vouga2", "Mondego1", "Mondego2", "Tagus", "Unknown")
